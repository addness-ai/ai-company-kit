...
| パート | できること | デプロイ | 目安 |
|---|---|---|---|
| **パート1：メール・カレンダー・スプレッドシート** | オフィスに自分のGmail・予定が出る／送信・予定作成／スプレッドシートの読み書き・新規作成 | **不要（ローカル）** | 約10分 |
...
# パート1：メール・カレンダー・スプレッドシート連携（ローカル・約10分）
...
5. 同じく「ライブラリ」→ **Google Sheets API** を「有効にする」
6. 「APIとサービス」→「OAuth同意画面」→ ユーザーの種類「外部」→ 作成
   - アプリ名（例：`AI会社`）と自分のメールを入力して保存・続行
   - 「テストユーザー」に**自分のGmailアドレスを追加**（重要）
7. 「APIとサービス」→「認証情報」→「認証情報を作成」→「OAuth クライアント ID」
   - 種類：**ウェブアプリケーション**
...
> これで完了。PCで本体を起動している間、メール・カレンダー・スプレッドシートが本物になります。

## 1-5. スプレッドシートの読み書き（Claude Code から）
`google:true` になっていれば、以下のAPIをClaudeが直接呼び出してスプレッドシートを操作できます。

- 読み取り（承認不要）：`GET /api/sheets/read?spreadsheetId=<ID>&range=<シート名!A1:F20>`
- 新規作成（承認必須）：`POST /api/actions/create-sheet` `{ title, headerRow, rows, approve }`
- 追記（承認必須）：`POST /api/actions/append-sheet` `{ spreadsheetId, range, rows, approve }`
- 上書き（承認必須）：`POST /api/actions/write-sheet` `{ spreadsheetId, range, rows, approve }`

作成・追記・上書きは、まず `approve` なしでリクエストして内容のプレビューを確認し、
社長のOKが出てから `approve:true` で再送する流れです（メール送信・予定作成と同じ承認フロー）。
既存シートの `spreadsheetId` は、シートのURL（`https://docs.google.com/spreadsheets/d/<ここ>/edit`）の部分です。
